#!/usr/bin/env python3
"""
C1: Standalone Signal Regression Diagnostic (self-generated, not cross-system parity)

Not run in CI, not pytest-collected. Compares AlphaLive's signal engine
output against a fixture generated by AlphaLive's OWN engine (see
tests/fixtures/generate_expected_signals.py's module docstring) - this is a
determinism/regression check against a previously-captured run of this same
codebase, not evidence that AlphaLive matches AlphaLab. For actual
cross-system parity evidence, see the CI-enforced
tests/test_multi_ticker_parity.py, whose fixtures are sourced from AlphaLab.
...
def load_expected_signals(strategy_name: str):
    ...
    if not signals_path.exists():
        raise FileNotFoundError(
            ...
            f"These are self-generated AlphaLive regression fixtures, not "
            f"AlphaLab parity evidence - see tests/fixtures/generate_expected_signals.py "
            f"--help. It requires an explicit --output-dir and explicit "
            f"--strategy/--all-strategies; it will not overwrite an existing "
            f"file without --overwrite. Regenerate deliberately into a scratch "
            f"directory first, review the diff, then copy into tests/fixtures/ "
            f"yourself if intended."
        )
