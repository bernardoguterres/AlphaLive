...
class TradeJournalGenerator:
    ...
    def export_to_csv(self, trades: List[Dict], output_file: str):
        ...
        # Write CSV
        fieldnames = [
            'trade_id', 'symbol', 'entry_date', 'entry_time', 'entry_price',
            'exit_date', 'exit_time', 'exit_price', 'qty', 'pnl_usd', 'pnl_pct',
            'hold_hours', 'hold_days', 'outcome', 'buy_order_id', 'sell_order_id'
        ]
    ...
    def print_summary(self, trades: List[Dict], stats: Dict):
        ...
        print(f"Winners: {stats['winners']} | Losers: {stats['losers']} | Break Even: {stats['break_even']}")
        print(f"Total P&L: ${stats['total_pnl_usd']:.2f} ({stats['total_pnl_pct']:.2f}%)")
        print(f"Avg P&L per Trade: ${stats['avg_pnl_usd']:.2f} ({stats['avg_pnl_pct']:.2f}%)")
        print(f"Avg Win: {stats['avg_win_pct']:.2f}% | Avg Loss: {stats['avg_loss_pct']:.2f}%")
        print(f"Largest Win: {stats['largest_win_pct']:.2f}% | Largest Loss: {stats['largest_loss_pct']:.2f}%")
        print(f"Avg Hold Time: {stats['avg_hold_hours']:.1f} hours ({stats['avg_hold_hours']/24:.1f} days)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate trade journal from Alpaca orders")
